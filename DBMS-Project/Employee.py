...
def add_to_cart():
    ...
    if (quantity_entry.get().isdigit()) or (quantity_entry.get() == ""):
        if (
                Items_1.get() != "" and quantity_entry.get() != "" and Items.get().lower() != "choose the category" and Items_1.get() != "Choose product"):
            n = Final_prod.index(Items_1.get())
            rate_n = Rate[n - 1]
            if Items.get() in category:
                invoice.insert("", index="end",
                               values=(Items_1.get(), quantity_entry.get(), rate_n, int(quantity_entry.get()) * rate_n))
                Items.current(0)
...
def save_bill():
    ...
    if dummy == 0:
        ...
        if yes_no == 1:
            cursor.execute("insert into customers values(?,?,?);", (cust_no, cust_name, cust_contact))
            dbconn.commit()
            Delete_btn.configure(state="active")
